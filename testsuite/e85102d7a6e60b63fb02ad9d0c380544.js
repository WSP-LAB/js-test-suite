...
{
    print_(dataView.toString());
    for (i in dataView)
    {
        print_(i + " == " + dataView[i]);
    };
    print_("array content");
    ...
    for (var i = 0; i < setFuncs.length; i++)
        ...
        if (offSet + dataSize[i] > dataView.byteLength)
            ...
            try {
                ...
                {
                print_("SUCCEEDED: exception " + e.description);
                succeeded = true;
                }
            if (!succeeded)
                {
                print_("failed to throw for out of bound access " + setFuncs[i] + " dataOffset is " + offSet);
                }
            }
        else
            {
            print_("set little endian value offset " + offSet + " value " + value + " method " + setFuncs[i]);
            print_("results of little endian reads are: ");
            dataView[setFuncs[i]](offSet, value, true);
            for (var j = 0; j < getFuncs.length; j++)
                {
                var result = GetResult(dataView, j, offSet, true);
                print_(getFuncs[j] + " = " + result);
                }
            print_("results of big endian reads are: ");
            for (var j = 0; j < getFuncs.length; j++)
                {
            ...
            print_("set big endian value offset " + offSet + " value " + value + " method " + setFuncs[i]);
            print_("results of little endian reads are: ");
